package net.hoys.turtlemod.world.dimension;
...
public class ModNetherPortal {
    ...
    public static Optional<ModNetherPortal> getNewPortal(WorldAccess world, BlockPos pos, Direction.Axis axis) {
        return getOrEmpty(world, pos, areaHelper -> areaHelper.isValid() && areaHelper.foundPortalBlocks == 0, axis);
    }

    public static Optional<ModNetherPortal> getOrEmpty(WorldAccess world, BlockPos pos, Predicate<ModNetherPortal> validator, Direction.Axis axis) {
        Optional<ModNetherPortal> optional = Optional.of(new ModNetherPortal(world, pos, axis)).filter(validator);
        if (optional.isPresent()) {
            return optional;
        } else {
            Direction.Axis axis2 = axis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
            return Optional.of(new ModNetherPortal(world, pos, axis2)).filter(validator);
        }
    }

    public ModNetherPortal(WorldAccess world, BlockPos pos, Direction.Axis axis) {
        this.world = world;
        this.axis = axis;
